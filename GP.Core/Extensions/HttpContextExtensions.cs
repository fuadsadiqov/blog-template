...
namespace GP.Core.Extensions
{
    public static class HttpContextExtensions
    {
        ...
        public static string GeApplicationHeaderValue(this HttpContext httpContext)
        {
            var result = httpContext.GetHeaderValueAs<string>("Application");

            if (string.IsNullOrEmpty(result)) result = ApplicationClaimValueConstants.Desktop;

            if (!(result.ToLower() == ApplicationClaimValueConstants.Mobile.ToLower() ||
                  result.ToLower() == ApplicationClaimValueConstants.Desktop.ToLower()))
                throw new Exception("Wrong http header");
