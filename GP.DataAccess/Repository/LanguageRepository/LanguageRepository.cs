...
namespace GP.DataAccess.Repository.LanguageRepository
{
    public class LanguageRepository : Repository<Language>, ILanguageRepository, IRepositoryIdentifier
    {
        ...
        {
            Expression<Func<GP.Domain.Entities.Lang.Language, bool>> filterPredicate = null;

            if (!string.IsNullOrEmpty(languageCode))
            {
                var isLanguageCodeExist = !string.IsNullOrEmpty(languageCode);
                filterPredicate = c => (!isLanguageCodeExist || c.Code.ToLower().Equals(languageCode.ToLowerInvariant()));
            }
            else
            {
