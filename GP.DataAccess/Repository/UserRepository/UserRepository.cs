...
namespace GP.DataAccess.Repository.UserRepository
{
    public class UserRepository : IUserRepository, IRepositoryIdentifier
    {
        ...
        public Task<User> GetUserByNameAsync(string userName, params Expression<Func<User, object>>[] includeProperties)
        {
            ...
            if (string.IsNullOrEmpty(userName))
                return Task.FromResult<User>(null);
            var query = _userManager.Users.ApplyQueryFilter(this._isQueryFilterApplied).AsSplitQuery().IncludeAll(includeProperties);
        ...
        {
            return _userManager.Users.ApplyQueryFilter(this._isQueryFilterApplied).AsSplitQuery().IncludeAll(includeProperties).Where(predicate);
        }

        public IQueryable<Domain.Entities.Identity.User> FindBy(params string[] includeProperties)
        {
            return _userManager.Users.ApplyQueryFilter(this._isQueryFilterApplied).AsSplitQuery().IncludeAll(includeProperties);
        }
        ...
        public void IncreaseOtpSentCount(User user)
        {
            user.OtpSentCount++;
        }
        public void ResetOtpSentCount(User user)
        {
            user.OtpSentCount = 0;
        }
    }
